...
class Server6 implements Runnable 
{
	...
	public void run() 
	{
		try {
			...
			ppt.load(freader);
			String P1port = ppt.getProperty("config.serverPort6");
			int P1servPort = Integer.parseInt(P1port);
			System.out.println(P1servPort);

			serverPSocket = new ServerSocket(P1servPort);
			System.out.println("Server6 Server started");
		} catch (Exception e) {
