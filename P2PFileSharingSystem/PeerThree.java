/**
* PeerServer3 class implements Server Functionality for a peer when a peer acts as a server for other Peer
* 
...
class PeerServer3 implements Runnable 
{
	private static ServerSocket serverPSocket;
	private static Socket peerclientSocket = null;
	//final static String peerDirectory3 = "/home/armaiv/varun/01Latest/Peer3/";
	private static String peerDirectory3=null;
	PeerServer3() 
	{
		try{
				...
				ppt.load(freader);
				peerDirectory3=ppt.getProperty("config.peerDirectory3");
	...
	public void run() 
	{
		try {
				...
				Properties ppt = new Properties();
				ppt.load(freader);
				String P3port=ppt.getProperty("config.P3serverPort");
				int P3servPort = Integer.parseInt(P3port);
				
				serverPSocket = new ServerSocket(P3servPort);
			...
			while (true) {
				try {
						...
						Thread t1 = new Thread(new PeerServerThread(peerclientSocket,peerDirectory3));
						t1.start();
...
/* PeerThree Class implements a peer functionality like Register files, Search File, Look Up Files Info and Download File */
public class PeerThree
{
	...
	public static void RegisterFiles() throws IOException 
	{
			...
			try {
					...
					File file = new File(peerDirectory3);
